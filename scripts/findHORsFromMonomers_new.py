...
if __name__ == '__main__':
    ...
    parser.add_argument("--dis-lev",type=bool, help="True for effective monomer distances, False for discrete monomer distances", required=False, default=True)
    ...
    monomer_lengths=[len(monomer) for monomer in monomers_as_features]
    mon_len=max(monomer_lengths, key=monomer_lengths.count)
    ...
    hor_tree = phylogeny_to_hor_tree(phylogeny, max_allowed_gap=max_gap, max_loop_size=maxHOR, min_loops=minL, allow_hor_overlap=False, set_branch_lengths=lev)
