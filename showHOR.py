...
def show_hor(hor_in_seq, tree=None, seq_length=None,
    ...
    fig = plt.figure(constrained_layout=True, figsize=(figure_width,len(clade_seq) * hor_item_height + tree_height))
    ...
    gs = fig.add_gridspec(
        ...
        height_ratios=[hor_item_size_ratio[0],tree_height_ratio,seq_height_ratio],
        hspace=h_space
    )
...
def show_hors(hors_in_seq, reference_seq,
    ...
    fig = plt.figure(constrained_layout=True, figsize=(fig_width,tree_height + hors_height))
    # fig.suptitle(f'HOR {label} ({bp_start}-{bp_end})')
    ...
    gs = fig.add_gridspec(
        len(hors_in_seq) + 1, 1,
        # width_ratios=[hor_size_ratio,1-hor_size_ratio],
        height_ratios=[hor_height for i in range(len(hors_in_seq))] + [tree_height],
        hspace=h_space
    )
